  ...
  return (
    <div className="min-h-screen text-[#e6e8ef] bg-[#0a0c12]">
            ...
            {session ? (
              <>
                ...
                <button
                  ...
                  className="rounded-xl px-3 py-1.5 text-sm bg-white/5 hover:bg-white/10"
                >
                  Sign out
                </button>
              </>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 md:py-16 space-y-8">
        {step === 'landing' ? (
          <Landing onCreate={onCreateClick} />
        ) : (
          <CreateTestForm email={session?.user?.email || ''} />
        )}
      </main>
  ...
  return (
    <section className="mx-auto max-w-3xl text-center space-y-6">
      <h1 className="text-4xl md:text-5xl font-semibold leading-tight">
        Test thumbnails and titles before you publish
      </h1>
      <p className="text-lg text-white/70">
        Share a unique link with your team or community. Collect quick votes to
        learn which combo wins attention.
      </p>
      <div className="pt-2">
        <button
          onClick={onCreate}
          className="inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium bg-gradient-to-br from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 shadow-lg shadow-indigo-900/20 ring-1 ring-white/10"
        >
          Create test
      ...
      <div className="w-full max-w-md rounded-3xl bg-[#0d1018] ring-1 ring-white/10 shadow-2xl overflow-hidden">
        ...
        <div className="p-5 sm:p-6 space-y-4">
          <button
            ...
            className="w-full rounded-xl bg-white text-black px-4 py-2.5 text-sm font-medium hover:opacity-90 transition"
          >
            Continue with Google
          </button>
          <p className="text-xs text-white/50">
            We use Google only to save your tests.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-10">
      <div className="space-y-2">
        ...
        <p className="text-white/60 text-sm">
          Signed in as <span className="text-white/80 font-medium">{email || 'guest'}</span>
        </p>
      </div>
        ...
        <div className="rounded-3xl p-5 sm:p-6 bg-white/[0.03] ring-1 ring-white/10 backdrop-blur-xl shadow-xl space-y-5">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. Episode 12 pre publish test"
              className="w-full rounded-xl bg-white/[0.03] border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-600"
            />
          </div>
        ...
        <div className="rounded-3xl p-5 sm:p-6 bg-white/[0.03] ring-1 ring-white/10 backdrop-blur-xl shadow-xl space-y-4">
          ...
          >
            <input
              ...
              className="hidden"
              onChange={onFiles}
              disabled={thumbs.length >= MAX_THUMBS}
            />
              ...
              {thumbs.length < MAX_THUMBS ? (
                <>
                  ...
                  <div className="text-white/50">
                    PNG or JPG, up to {MAX_THUMBS - thumbs.length} more
                  </div>
                </>
              ) : (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {thumbs.map((th) => (
                ...
                >
                  ...
                  />
                  <button
                    onClick={() => removeThumb(th.id)}
                    className="absolute top-2 right-2 rounded-lg bg-black/50 p-1 text-white/80 hover:bg-black/70"
                  >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {!canPublish && (
          <span className="text-sm text-white/60">
            Add at least one title and one thumbnail
          </span>
        )}
      </div>
    </div>
